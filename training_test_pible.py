...
def test_and_print_results(folder, iteration, start_train, end_train, start_test, end_test, sc_volt_start_test):
    Agnt = "PPO"
    pattern = subprocess.getoutput('eval echo "~$USER"') + '/ray_results/' + folder + '/checkpoint_' + str(
        iteration) + '/checkpoint-' + str(iteration)
    ...
    diff_days = datetime.datetime.strptime(end_test, '%m/%d/%y %H:%M:%S') - datetime.datetime.strptime(start_test,
                                                                                                       '%m/%d/%y %H:%M:%S')
    ...
    while True:
        ...
        if stop >= repeat * (diff_days.days / (1)):
            ...
            break
    print("Energy Prod: ", energy_prod_tot / (repeat * diff_days.days), "Energy Used: ",
          energy_used_tot / (repeat * diff_days.days))
    ...
    Pible_func.write_results(tot_rew, start_train, end_train, start_test, end_test, percent, diff_days.days,
                             energy_prod_tot / (repeat * diff_days.days), energy_used_tot / (repeat * diff_days.days),
                             res["events_detect"],
...
if __name__ == "__main__":
    ...
    while True:
        ...
        sleep(10)
        resume_path, sc_volt_start_test = test_and_print_results(folder, iteration, start_train, end_train_temp,
                                                                 start_test, end_test, sc_volt_start_test)
