...
def light_event_func(t_now, next_wake_up_time, mode, PIR_on_off, events_found_dict, count, light_prev, file_data): # check how many events are on this laps of time
    event_gt = 0
    event_found = 0
    PIR = 0
    light = 0
...
def reward_func_high_level(PIR_on_Off, mode, event, SC_volt, next_wake_up):
    reward = 0; detect = 0
    miss = np.nan
    '''
...
def find_agent_saved(path):
    ...
    for count, line in enumerate(open(path + "/" + folder + "/result.json", 'r')):
        ...
        if dict['episode_reward_min'] >= max_mean and count > int(tot_iterations/2):
