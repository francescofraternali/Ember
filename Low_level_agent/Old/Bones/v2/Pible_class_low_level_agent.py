...
class SimplePible(gym.Env):
    ...
    def __init__(self, config):
        ...
        self.Reward = []; self.Mode = []; self.Time = []; self.Light = []; self.PIR_OnOff_hist = []; self.SC_Volt = []; self.State_Trans = []; self.thpl_event_det_hist = []; self.thpl_event_miss_hist = []
    ...
    def step(self, action):
        ...
        if self.train == 'test':
            self.Reward.append(reward); self.Time.append(self.time); self.Mode.append(self.mode); self.Light.append(self.light); self.PIR_OnOff_hist.append(action); self.SC_Volt.append(SC_temp);
    ...
    def render(self, tot_rew, title):
        Pible_func.plot_hist_low_level(self.Time, self.Light, self.Mode, self.PIR_OnOff_hist, self.State_Trans, self.Reward, self.SC_Volt, self.PIR_event_det_hist, self.PIR_event_miss_hist, self.thpl_event_det_hist, self.thpl_event_miss_hist, tot_rew, self.PIR_tot_events_detect, self.PIR_tot_events, self.Len_Dict_Events, title)
