...
def test_and_print_results(folder, iteration, start_test, end_test, title, curr_path):
    ...
    while True:
        ...
        print(round(float(info["energy_used"]), 5), round(reward, 5), float(info["PIR_events_detect"]), float(info["thpl_events_detect"]))
    ...
    env.render(tot_rew, "RL_Sens_ST_few_events", energy_used_tot)
    return path
