...
#logging.basicConfig(filename='/mnt/c/Users/Francesco/Dropbox/EH/RL/RL_MY/Ember/HRL/example.log',level=logging.DEBUG)

class WindyMazeEnv(gym.Env):
    def __init__(self, env_config):
        print("init WindyMazeEnv")
        logging.warning('Watch out!')
        #sleep(0.1)
        self.map = [m for m in MAP_DATA.split("\n") if m]
        self.x_dim = len(self.map)
        self.y_dim = len(self.map[0])
        logging.warning("Loaded map {} {}".format(self.x_dim, self.y_dim))
        for x in range(self.x_dim):
            for y in range(self.y_dim):
                if self.map[x][y] == "S":
                    ...
                    self.end_pos = (x, y)
        logger.warning("Start pos {} end pos {}".format(self.start_pos,
                                                     self.end_pos))
    ...
    def step(self, action):
        #print("step WindyMazeEnv, action: ", action)
        #sleep(0.1)
        if action == 1:
        ...
        if done:
            print(at_goal, self.num_steps)
        return ([[self.pos[0], self.pos[1]], self.wind_direction],
                100 * int(at_goal), done, {})

    def _get_new_pos(self, pos, direction):
        #print("get_new_pos WindyMazeEnv")
        #sleep(0.1)
        if direction == 0:
...
class HierarchicalWindyMazeEnv(MultiAgentEnv):
    def __init__(self, env_config):
        #print("init HierarchicalWindyMazeEnv")
        #sleep(0.1)
        self.flat_env = WindyMazeEnv(env_config)

    def reset(self):
        #print("reset HierarchicalWindyMazeEnv")
        #sleep(1)
    ...
    def step(self, action_dict):
        #print("step HierarchicalWindyMazeEnv")
        #sleep(0.1)
        #print("action_dic ", action_dict)
    ...
    def _high_level_step(self, action):
        #print("in higher HierarchicalWindyMazeEnv, action: ", action)
        #logger.warning("High level agent sets goal {}".format(action))
        #sleep(0.1)
    ...
    def _low_level_step(self, action):
        #print("in lower HierarchicalWindyMazeEnv, action: ", action)
        #sleep(1)
        #logger.warning("Low level agent step {}".format(action))
        ...
        if f_done:
            #print("f_done HierarchicalWindyMazeEnv")
            #sleep(5)
            done["__all__"] = True
            logger.warning("high level final reward {}".format(f_rew))
            if f_rew == 100:
                print("obs, action, goal_pos, rew, done", obs, action, goal_pos, rew, done)
                sleep(1)
            rew["high_level_agent"] = f_rew
            obs["high_level_agent"] = f_obs
        elif self.steps_remaining_at_level == 0:
            #print("elif f_done HierarchicalWindyMazeEnv")
            #sleep(0.1)
